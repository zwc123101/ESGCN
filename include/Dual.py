...
def add_diag_layer(inlayer, dimension, M, act_func, dropout=0.0, init=ones):
    ...
    w0 = init([1, dimension])
    # tosum = tf.sparse_tensor_dense_matm2ul(M, tf.multiply(inlayer, w0))
    tosum = tf.sparse.sparse_dense_matmul(M, tf.multiply(inlayer, w0))
    if act_func is None:
        return tosum
    else:
...
def add_full_layer(inlayer, dimension_in, dimension_out, M, act_func, dropout=0.0, init=glorot):
    ...
    tosum = tf.sparse.sparse_dense_matmul(M, tf.matmul(inlayer, w0))
    if act_func is None:
        return tosum
    else:
        return act_func(tosum)
...
def add_sparse_att_layer(inlayer, dual_layer, r_mat, act_func, e):
    ...
    vals = tf.sparse.sparse_dense_matmul(coefs, inlayer)
    if act_func is None:
        return vals
    else:
...
def add_self_att_layer(inlayer, adj_mat, act_func, hid_dim):
    ...
    # in_fts = tf.keras.layers.Conv1D(tf.expand_dims(inlayer, 0), hid_dim, 1, use_bias=True)
    f_1 = tf.reshape(tf.layers.conv1d(in_fts, 1, 1), (-1, 1))
    # f_1 = tf.reshape(tf.keras.layers.Conv1D(in_fts, 1, 1), (-1, 1))
    f_2 = tf.reshape(tf.layers.conv1d(in_fts, 1, 1), (-1, 1))
    # f_2 = tf.reshape(tf.keras.layers.Conv1D(in_fts, 1, 1), (-1, 1))
    logits = f_1 + tf.transpose(f_2)
...
def get_input_layer(e, dimension, lang):
    ...
    with open(file=lang, mode='r', encoding='utf-8') as f:
...
def get_loss(outlayer, ILL, gamma, k):
    ...
    neg_left = tf.compat.v1.placeholder(tf.int32, [t * k], "neg_left")
    # neg_right = tf.placeholder(tf.int32, [t * k], "neg_right")
    neg_right = tf.compat.v1.placeholder(tf.int32, [t * k], "neg_right")
...
def build_1(dimension, act_func, alpha, beta, gamma, k, lang, e, ILL, KG):
    ...
    print('shape of output_h: ', output_layer.get_shape())
